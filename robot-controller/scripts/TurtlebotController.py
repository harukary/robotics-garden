...
class RobotController:
    ...
    def __init__(self, topics):
        ...
        # Sub
        if 'pose' in topics.keys():
            self.pose = None
            self.pose_sub = rospy.Subscriber(topics['pose'], PoseStamped, self.pose_callback)
        if 'odom' in topics.keys():
            self.odom = None
            self.odom_sub = rospy.Subscriber(topics['odom'], PoseStamped, self.odom_callback)
        if 'image' in topics.keys():
            cv2.namedWindow("image", cv2.WINDOW_NORMAL)
            self.image_sub = rospy.Subscriber(topics['image'],Image,self.image_callback)
        if 'compressed_image' in topics.keys():
            cv2.namedWindow("compressed_image", cv2.WINDOW_NORMAL)
    ...
    def nav_callback(self, msg):
        if msg.status.status == 3 and self.nav_state == 'moving':
